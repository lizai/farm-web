...
@Repository
@Transactional
public class DishDaoImpl extends GenericDaoHibernate<Dish, Long> implements IDishDao {

    private static final String identityKey = Dish.PROPERTYNAME_DISH_ID;
    ...
    @Override
    public void saveOrUpdateEntity(Dish book) {
    ...
    public List getList(String orderColumn, boolean ifDesc, int start, int offset, List<Long> receivedIds) {
        DetachedCriteria query = DetachedCriteria.forClass(Dish.class);
    ...
    @Override
    public List getListInIds(List<Long> ids) {
        DetachedCriteria query = DetachedCriteria.forClass(Dish.class);
        if (ids != null && ids.size() > 0) {
            Criterion criterion = Restrictions.in("dishId", ids);
            query.add(criterion);
