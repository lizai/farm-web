...
@Repository
@Transactional
public class CommodityDaoImpl extends GenericDaoHibernate<Commodity, Long> implements ICommodityDao {

    private static final String identityKey = Commodity.PROPERTYNAME_COMMODITY_ID;
    ...
    @Override
    public void saveOrUpdateEntity(Commodity commodity) {
    ...
    public List getListInIds(List<Long> ids) {
        ...
        if (ids != null && ids.size() > 0) {
            Criterion criterion = Restrictions.in("commodityId", ids);
            query.add(criterion);
