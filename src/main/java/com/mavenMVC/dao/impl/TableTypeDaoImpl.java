...
@Repository
@Transactional
public class TableTypeDaoImpl extends GenericDaoHibernate<Tabletype, Long> implements ITableTypeDao {

    private static final String identityKey = Tabletype.PROPERTYNAME_TABLE_TYPE_ID;

    public TableTypeDaoImpl() {
        super(Tabletype.class);
    }

    @Override
    public Tabletype getById(Long id) {
        return get(id);
    }

    @Override
    public void saveOrUpdateEntity(Tabletype book) {
    ...
    public List getList(String orderColumn, boolean ifDesc, int start, int offset, List<Long> receivedIds) {
        DetachedCriteria query = DetachedCriteria.forClass(Tabletype.class);
