...
public class OrderDaoImpl extends GenericDaoHibernate<Order, Long> implements IOrderDao {
    ...
    private static final String identityKey = Order.PROPERTYNAME_ORDER_ID;
    ...
    public List getList(String orderColumn, boolean ifDesc, int start, int offset, List<Long> receivedIds) {
        ...
        if(orderColumn != null && !orderColumn.trim().equals("")){
            query.addOrder(ifDesc ? org.hibernate.criterion.Order.desc(orderColumn) : org.hibernate.criterion.Order.asc(orderColumn));
        }
